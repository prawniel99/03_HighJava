...
public class N03UdpServer {

    public static void main(String[] args) {
        try {
            // 통신포트를 지정하여 소켓 객체를 생성한다. 포트 번호 자유.
            DatagramSocket socket = new DatagramSocket(9999);
            System.out.println("서버 실행 중");

            while(true) {
                // 전송 받은 메시지가 저장될 변수 선언
                ...
                // 전송 받으면 바이트 배열에 메시지가 들어가면서 그 배열이 이 패킷에 들어온다?

                // 데이터를 수신한다 ==> 소켓 객체의 receive() 메소드를 사용한다.
                //                 ==> 이 메소드에 수신용 패킷 객체를 인수값으로 넣어준다.
                socket.receive(inPacket); // ==> 수신된 데이터의 패킷 정보가 인수값으로 지정한 패킷변수(inPacket)에 저장된다.
                ...
                // 방법2) 수신용 패킷 객체의 getData() 메소드를 이용한다 ==> 수신한 데이터를 byte형 배열로 반환한다.
                ...
                // 상대방에게 메시지 보내기 (예: 수신받은 데ㅣ터를 그대로 송신하기())
                ...
                // 송신 작업 ==> 소켓 객체의 send() 메소드를 이용한다. ==> 이 메소드의 매개변수에 송신용 패킷객체를 넣어준다.
