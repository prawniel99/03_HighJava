...
@WebServlet("/qnaInsertReply.do")
public class QnaInsertReply extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 역직렬화 - 자바 객체로
		Gson gson = new Gson();
		QnaVO rvo = gson.fromJson(reqData, QnaVO.class);
		
		// service 객체 얻기
		QnaService service = QnaService.getInstance();
		
		// service 메소드 호출 - 결과값
		int cnt = service.qnaInsertReply(rvo);
