...
@WebServlet("/prod/prodAccList.do")
public class ProdAccList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//페이지네이션 서비스 호출
		PageVO pageInfo = prodService.getAccPagination(page, stype, sword);
		...
		//이제 이걸 쓰면안되고
		//List<ProdVO> prodList = prodService.selectForAcc();
		
		// 상품리스트 페이지네이션 조회 이걸써야함
		List<ProdVO> prodList = prodService.getPaginatedAccList(paramMap);
		...
		request.getRequestDispatcher("/main?view=prodAccList").forward(request, response);
	}
