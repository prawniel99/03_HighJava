...
%>
    <a href="<%=request.getContextPath()%>/prod/prodAccList.do?page=<%=pageInfo.getStartPage() - 1%>&stype=<%=request.getParameter("stype")%>&sword=<%=request.getParameter("sword")%>">이전</a>
<%
        }
        for (int i = pageInfo.getStartPage(); i <= pageInfo.getEndPage(); i++) {
%>
    <a href="<%=request.getContextPath()%>/prod/prodAccList.do?page=<%=i%>&stype=<%=request.getParameter("stype")%>&sword=<%=request.getParameter("sword")%>"><%=i%></a>
<%
        }
        if (pageInfo.getEndPage() < pageInfo.getTotalPage()) {
%>
    <a href="<%=request.getContextPath()%>/prod/prodAccList.do?page=<%=pageInfo.getEndPage() + 1%>&stype=<%=request.getParameter("stype")%>&sword=<%=request.getParameter("sword")%>">다음</a>
<%
        }
    }
%>
...
<div class="search-bar">
    <form action="<%= request.getContextPath() %>/prod/prodAccList.do" method="get">
        <select name="stype">
