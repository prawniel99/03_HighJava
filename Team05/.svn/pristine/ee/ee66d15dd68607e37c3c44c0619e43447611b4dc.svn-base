...
public class ProdNewArrList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//페이지네이션 서비스 호출
		PageVO pageInfo = prodService.getNewArrPagination(page, stype, sword);
		...
		List<ProdVO> prodList = prodService.getPaginatedNewArrivals(paramMap);
		...
		System.out.println("신상품 prodList 조회 : " + prodList);
		System.out.println("신상품 pageInfo 조회 : " + pageInfo);
		System.out.println("신상품 stype 조회 : " + stype);
		System.out.println("신상품 sword 조회 : " + sword);
		
		request.getRequestDispatcher("/main?view=prodNewArrList").forward(request, response);
	}
